...
func (u UserRoleRepository) GetUserRoleByID(i int64) (*entity.UserRole, error) {
	...
	_, newlogger := logger.GetLogger().Start(u.c, "GET_USER_ROLE_BY_ID: DATABASE")
	defer newlogger.End()
	newlogger.Info("STARTING: GET USER ROLE BY ID", map[string]interface{}{"id": i})
	var userRole entity.UserRole
	if err := u.db.Debug().Model(&entity.UserRole{}).Where("id = ?", i).First(&userRole).Error; err != nil {
		newlogger.Error("GET_USER_BY_ID: ERROR", map[string]interface{}{"error": err.Error()})
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, payload.ErrEntityNotFound("user_roles", err)
		}
		return nil, payload.ErrDB(err)
	}
	newlogger.Info("GET_USER_BY_ID: SUCCESSFULLY", map[string]interface{}{"user_role": userRole})
	return &userRole, nil
}
